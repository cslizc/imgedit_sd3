...
from instp2p_sd3_pl import StableDiffusion3InstructPix2PixPipeline
...
DATASET_NAME_MAPPING = {
    ...
    "osunlp/MagicBrush": ("source_img", "target_img", "instruction"),
}
...
def main(args):
    ...
    logger.info("Initializing the InstructPix2Pix Transformer from the pretrained Transformer.")
    in_channels = 32
    ...
    with torch.no_grad():
        ...
        new_conv_in.weight[:, :16, :, :].copy_(transformer.pos_embed.proj.weight)
        new_conv_in.bias.copy_(transformer.pos_embed.proj.bias)
        transformer.pos_embed.proj = new_conv_in
    ...
    if not "train" in train_dataset.keys():
        train_dataset = DatasetDict({"train": datasets.concatenate_datasets(
    ...
    def preprocess_train(examples):
        examples_new = {}
        examples_new["original_pixel_values"] = [preproc_img(image) for image in examples[original_image_column]]
        examples_new["edited_pixel_values"] = [preproc_img(image) for image in examples[edited_image_column]]
        examples_new["instance_prompt"] = examples[edit_prompt_column]
        return examples_new
    ...
    with accelerator.main_process_first():
        ...
        train_dataset = train_dataset["train"].with_transform(preprocess_train)
    ...
    if False:#not args.train_text_encoder and not train_dataset.custom_instance_prompts:
    ...
    if False:#not train_dataset.custom_instance_prompts:
        if not args.train_text_encoder:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                #latents = batch["edited_pixel_values"].to(dtype=vae.dtype)
                ...
                #print("edited_pixel_values", batch["edited_pixel_values"].size())
                model_input = vae.encode(batch["edited_pixel_values"].to(dtype=vae.dtype)).latent_dist.sample()
                ...
                original_image_embeds = vae.encode(batch["original_pixel_values"].to(dtype=vae.dtype)).latent_dist.mode()
                #print("b4 prompt_embeds", prompt_embeds.size())
                #print("b4 pooled_prompt_embeds", pooled_prompt_embeds.size())
                if args.conditioning_dropout_prob is not None:
                    random_p = torch.rand(bsz, device=model_input.device, generator=generator)
                    # Sample masks for the edit prompts.
                    prompt_mask = random_p < 2 * args.conditioning_dropout_prob
                    pool_prompt_mask = prompt_mask.reshape(bsz, 1)
                    prompt_mask = prompt_mask.reshape(bsz, 1, 1)
                    
                    # Final text conditioning.
                    #null_conditioning = text_encoder(tokenize_captions([""]).to(accelerator.device))[0]
                    null_prompt_embeds, null_pooled_prompt_embeds = compute_text_embeddings(
                        ["" for _ in prompts], text_encoders, tokenizers
                    )
                    prompt_embeds = torch.where(prompt_mask, null_prompt_embeds, prompt_embeds)
                    pooled_prompt_embeds = torch.where(pool_prompt_mask, null_pooled_prompt_embeds, pooled_prompt_embeds)
                ...
                #print("a5 prompt_embeds", prompt_embeds.size())
                #print("a5 pooled_prompt_embeds", pooled_prompt_embeds.size())
                # Concatenate the `original_image_embeds` with the `noisy_latents`.
                #print("noisy_model_input", noisy_model_input.size())
                #print("original_image_embeds", original_image_embeds.size())
                #print("prompt_embeds", prompt_embeds.size())
                concatenated_noisy_input = torch.cat([noisy_model_input, original_image_embeds], dim=1)
                #print('latent size', transformer.pos_embed(original_image_embeds).size())
                # Predict the noise residual
                if not args.train_text_encoder:
                    model_pred = transformer(
                        hidden_states=concatenated_noisy_input,
                        timestep=timesteps,
